...
void prtMiWin(int ptm, int w) {
    printf(" %c miai-win, %c to move ...", emit(w), emit(ptm));
}
...
int upConn(Board& B, int st, bool uzMi, Move h[], int mvs) {
    int bdst;
    ...
    for (int j = 0; j < mvs; j++)  // now update connectivity
        if (st == h[j].s) B.move(h[j], uzMi, bdst);
    return bdst;
}

void mvPlsUpdt(Board& B, Move mv, Move h[], int& m, bool uzMi, int& bdst,
               bool& w) {
...
void undoMove(Board& B, Move h[], int& moves, bool uzMi, bool& w) {
    if (moves > 0) {  // start from scratch, replay all but last move
        ...
        upConn(B, opt(pX), uzMi, h,
               moves - 1);                  // place pY stones, update pY conn
...
ScoreLcn easyMove(Board& B, int st, Move h[], int mvs,
                  bool v) {        // win vc prior to mv, so...
    prtMiWin(st, st);              // ... quiet move is opnt-best-move
    upConn(B, st, false, h, mvs);  // update conn'y with no miai
    Playout pl(B);
    int r = ROLLOUTS;
    ScoreLcn osl = flat_MCS(r, B, pl, opt(st), false, true,
                            v);  // best **opnt** move (no miai, yes accel)
    return ScoreLcn(MAXSCORE - osl.scr, osl.lcn);
}

ScoreLcn futileMove(Board& B, int st, Move h[], int mvs,
                    bool v) {  // miai loss before moving
    prtMiWin(st, opt(st));
    upConn(B, st, false, h, mvs);       // update conn'y with no miai
    ...
    while (!quit) {
        ...
        switch (cmd) {
            case QUIT_CH:
                ...
                break;
            case PLAYHEX_CH:
                playHex(B, h, moves, st, lcn);
                break;  // st,lcn used for x,y
            case UNDO_CH:
                undoMove(B, h, moves, uzMi, abswin);
                B.showAll();
                break;
            case GENMOVE_CH:
                if (abswin) {
                ...
                if (has_win(upConn(B, st, uzMi, h, moves))) {
                    sl = easyMove(B, st, h, moves, v);
                    lcn = sl.lcn;
                } else if (has_win(upConn(B, opt(st), true, h, moves))) {
                    sl = futileMove(B, st, h, moves, v);
                    lcn = sl.lcn;
                } else {
                    Playout pl(B);
                    int r = ROLLOUTS;
                    ...
                    if (pl.mpsz == 0)  // mustplay 0, search without miai
                    {
                        r = ROLLOUTS;
                        Playout pq(B);
                        sl = flat_MCS(r, B, pq, st, false, acc, v);
                    }
                    lcn = sl.lcn;
                }
                ...
                // lcn = rand_move  (B);
                printf("score %.2f\n", scrToFlt(sl.scr));
                prtMoveMade(Move(st, lcn));  // continue ...
            default:                         // specified move
                if (abswin) {
                    prtGameOver();
                    break;
                }
                if ((EMP != st) && (EMP != B.board[lcn])) {
                    prtIllegal();
                    break;
                }
