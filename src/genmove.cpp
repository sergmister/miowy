...
int Playout::eval(int st, int maxSims) {
    int p = nx(st);
    int sims = wins[0] + wins[1];
    assert(sims > 0);
...
void topKLcns(int topKL[], int& k,
              int val[]) {  // lcns of top k vals, sorted by val[]
    int local[TotalGBCells];
    int dummy;
    copyvec(val, TotalGBCells, local, dummy);
    int j;
    for (j = 0; j < k; j++) {
...
ScoreLcn ngmx_MCS(int r, Board& Bd, int s, bool rtClr, int d, int w, int alf,
                  int bet, bool v) {
    bool uzM = true;
    bool acc = true;
    int Children[TotalCells];
    int numCh;
    Board B = Bd;
    Playout pl(B);
    ...
    assert(pl.mpsz > 0);
    if (pl.mpsz == 1) {
        Children[0] = rootsl.lcn;
        numCh = 1;
    } else {
        numCh = w;
        if (pl.mpsz < pl.numAvail) numCh = pl.mpsz;
        printf("w %d pl.mpsz %d min %d\n", w, pl.mpsz, numCh);
        topKLcns(Children, numCh,
                 pl.AMAF[nx(s)]);  // flat_MCS erased non-mp vals
    }
    ...
    for (int j = 0; j < numCh; j++) {
        ...
        int chMiai;
        int chbdst = BRDR_NIL;
        Move chMv(s, Children[j]);
        chMiai = chB.move(chMv, uzM, chbdst);
        chB.show();
        ScoreLcn chSL = ngmx_MCS(r, chB, opt(s), !rtClr, d - 1, w, negScr(bet),
                                 negScr(alf), v);
        chSL.scr = negScr(chSL.scr);
        printf("tried move %c ", emit(opt(s)));
        prtLcn(chSL.lcn);
        printf(" %.2f\n", scrToFlt(chSL.scr));
        if (chSL.scr > bstSL.scr) {
            bstSL.scr = chSL.scr;
            bstSL.lcn = Children[j];
        }
...
void printInfo(Playout& pl, int s, int r) {
    ...
    printf("sims %d  %c wins %.2f eval.len %.2f ", r, emit(s),
           float(mywins) / float(r), (0.5 + olen) / (mylen + olen));
    printf("len %2.2f (opt %2.2f) minlen %d (%d)\n\n", ratio(mysum, mywins),
           ratio(osum, owins), pl.minwinlen[nx(s)], pl.minwinlen[nx(opt(s))]);
}

int indexOf(int x, int A[], int n) {
    int j;  // return index of x in A[0]...A[n-1]
...
void refine_MP(Board& B, Playout& pl, Move killer, bool useMiai, bool vrbs) {
    ...
    int T[TotalCells];
    int threats = 1;  // ... threats start at 1
    int C[TotalCells];
    int Csz;
    int M[TotalCells];
    int Msz;
    if (vrbs) printf("\nrefine mustplay %c \n", emit(killer.s));
    for (int r = 0; r < N;
         r++) {  // check every possible move for win, if yes, refine MP
        for (int c = 0; c < N - r; c++) {
            int newlcn = fatten(r, c);
            if (newlcn !=
                killer.lcn) {  // if equal, we already have the carrier...
                Move mv(opt(killer.s), newlcn);
                if ((B.board[newlcn] == EMP) &&
                    (is_win(B, mv, useMiai, M, Msz, vrbs))) {  // new threat
                    T[threats++] = newlcn;
...
void myshuffle(Playout& pl, int& end_w, const int& j_w, const bool& acc) {
    if (!acc)
        ...
        if (j_w > end_w) {  // add to winners; shuffle remainders
            swap(pl.Avail[++end_w],
                 pl.Avail[j_w]);  // add new winner to end of sublist
            shuffle_interval(pl.Avail, end_w + 1,
                             pl.numAvail - 1);  // shuffle nonwinners
        }
...
void threatInit(Playout& pl, int& k) {  // cycle through mustplay as move[0]
    ...
    assert(k >= 0);
    if (k >= pl.mpsz) {
        printf("resetting MPndx\n");
        k = 0;
    }
    int ndx = indexOf(pl.MP[k], pl.Avail, TotalCells);
    swap(pl.Avail[0], pl.Avail[ndx]);  // next mustplay element
    k++;
    if (k == pl.mpsz) k = 0;  // increment k, cyclically
}

int sortMPLcn(int MP[], int MPsize, int s,
              int A[2][TotalGBCells]) {  // sort by A[][] val
...
ScoreLcn goodMove(Playout& pl, int s, int sims, int maxSims, bool v) {
    ScoreLcn sl;
    assert(sims == pl.wins[0] + pl.wins[1]);
    if (pl.mpsz == 0) {
        if (v) printf("mustplay 0 after %d sims\n", sims);
        sl.scr = 0;
        sl.lcn = pl.MP[0];
    ...
    if (pl.mpsz == 1) {
        if (v) printf("mustplay 1 after %d sims\n", sims);
    ...
    if (pl.mpsz < pl.numAvail) {  // nontrivial mustplay, zero out other scores
        ...
        for (int x = 0; x < N; x++)
            for (int y = 0; y < N - x; y++) inMP[fatten(x, y)] = false;
        for (int m = 0; m < pl.mpsz; m++) inMP[pl.MP[m]] = true;
        for (int x = 0; x < N; x++)
            for (int y = 0; y < N - x; y++) {
                int psn = fatten(x, y);
                if (!inMP[psn]) {
                    erase(pl.AMAF[nx(s)][psn]);
                    erase(pl.AMAF[nx(opt(s))][psn]);
                }
            }
    }
    if (pl.wins[nx(s)] > 0) {
...
ScoreLcn flat_MCS(int& r, Board& B, Playout& pl, int s, bool uzMi, bool acc,
                  bool v) {  // mustplay left at front of Avail
    ...
    for (int j = 0; j < r; j++) {
        ...
        // if ((j<2)&&(v))
        // prtPlayoutMsg(j,emit(turn),pl.Avail[just_won],just_won); // data for
        // user
