...
bool valid_psn(int psn) { return ((psn > 0) && (psn <= N)) ? true : false; }
...
bool expect(char ch0, char ch1) { return (ch0 == ch1); }
...
void parseGivenMv(char& cmd, char ch, bool& ok, int& lcn) {
    int x, y;
    cmd = THISMOVE_CH;
    ok = islower(ch);
    if (ok) {
    ...
    do {
        ...
        switch (ch) {
            ...
            case UNDO_CH:
                ...
                break;
            case PLAYHEX_CH:
                parsePH(cmd, ch, ok, s, lcn);
                break;
            case BLK_CH:
            case WHT_CH:
                parseMove(cmd, ch, ok, s, lcn);
                break;
            default:
                nextEOL(ch);
                printf("sorry, what?\n");
