...
int Node::uct_playout(Board& B, int plyr, bool useMiai) {
    if (isLeaf())
        if (stat.n >= EXPAND_THRESHOLD) expand(B, plyr);
    int winner;
    if (isLeaf()) {
        Playout pl(B);
        winner = plyr;
        ...
        if (miReply != c.lcn) {
            ...
            B.zero_connectivity(opt(plyr),
                                false);  // ... but do not remove stones
            // B.showBr();
            // now use stones to update connectivity
            int opponent_bdst;
            for (int j = 0; j < TotalGBCells; j++)
                if (B.board[j] == TMP) {
                    // prtLcn(j); printf(" conn update plyr
                    // %c\n",emit(opt(plyr)));
                    // need to update opponent's board, but opponent_bdst will
                    // not be used
                    // TODO: move the recomputation of board connectivity into
                    // board
...
void Node::expand(Board& B, int s) {
    ...
    c = 0;
    for (int j = 0; j < TotalGBCells; j++)
        if (B.board[j] == EMP) children[c++].lcn = j;
}
