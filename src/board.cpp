...
void prtLcn(int psn) { printf("%c%1d", alphaCol(psn), numerRow(psn)); }
...
int b2(int shape, int j) {  // inner loop bound in shapeAs
    switch (shape) {
        case RHOMBUS:
            return Np2G;  // will display as Np2G*Np2G rhombus
        case TRI:
            return N - j;  // will display as N*N*N triangle
        default:
            assert(0 == 1);
            return 0;
    }
...
void shapeAs(int shape, int X[]) {  // display as given shape
    int psn = 0;
    int b = (shape == RHOMBUS) ? Np2G : N;
    for (int j = 0; j < b; j++) {
        for (int k = 0; k < j; k++) printf(" ");
        for (int k = 0; k < b2(shape, j); k++) printf(" %3d", X[psn++]);
        printf("\n");
    }
    printf("\n");
}
...
void showBothYcore(int X[], int Y[]) {
    int j, k, psn, x;
...
void display_nearedges() {
    int T[TotalGBCells];
    for (int j = 0; j < TotalGBCells; j++) T[j] = 0;
    for (int j = 0; j < TotalGBCells; j++)
        if ((board_row(j) >= 0) && (board_col(j) >= 0) && (board_row(j) < N) &&
            (board_col(j) < N) && (board_row(j) + board_col(j) < N) &&
            near_edge(j))
            T[j] = 9;
    shapeAs(RHOMBUS, T);
}
...
void Playout::single_playout(int& trn, int& k, bool useMiai) {
    ...
    do {
        ...
        if (useMiai) {
            ...
            if (resp != Avail[k]) {
                int z;  // prep for autorespond on next move
    ...
    if (k == 1) {
        if (!useMiai) {
            // if (v) { prtLcn(Avail[1]); printf(" is killer, so mp
            // singleton\n"); }
            mpsz = 1;
            MP[0] = Avail[1];
            ...
            int oldmpsz = mpsz;
            int oldMP[TotalCells];
            int newmpsz;
            int newMP[TotalCells];
...
Playout::Playout(Board& B) : B(B) {
    ...
    for (int r = 0; r < N; r++)
        for (int c = 0; c < N - r; c++) {
            psn = fatten(r, c);
            if (EMP == B.board[psn]) Avail[j++] = psn;
        }
...
int Board::num(int kind) {
    int count = 0;
    for (int j = 0; j < N; j++)
        for (int k = 0; k < N; k++)
            if (board[fatten(j, k)] == kind) count++;
    return count;
}
...
void Board::showBr() {
    printf("  brdr (top 1   left 2   right 4)\n");
    showYcore(brdr);
}
...
void Board::init() {
    ...
    for (j = 0; j < TotalGBCells; j++) {
        ...
        brdr[j] = BRDR_NIL;
        p[j] = j;
        for (k = 0; k < 2; k++) reply[k][j] = j;
    }
    for (j = 0; j < N; j++)
        for (k = 0; k < N - j; k++) board[fatten(j, k)] = EMP;
    for (j = 0; j < N + 1; j++) brdr[fatten(0, 0) + j - Np2G] = BRDR_TOP;
    for (j = 0; j < N + 1; j++) brdr[fatten(0, 0) + j * Np2G - 1] = BRDR_L;
    for (j = 0; j < N + 1; j++)
