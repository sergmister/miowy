...
const int
    Nbr_offsets[NumNbrs + 2]  // wraparound, avoid mod, can always access [x+2]
    = {-Np2G, 1 - Np2G, 1, Np2G, Np2G - 1, -1, -Np2G, 1 - Np2G};
const int Bridge_offsets[NumNbrs +
                         2]  // wraparound, avoid mod, can always access [x+2]
    = {-2 * Np2G + 1, 2 - Np2G,    Np2G + 1,      2 * Np2G - 1,
       Np2G - 2,      -(Np2G + 1), -2 * Np2G + 1, 2 - Np2G};
...
void show_winners(struct Board B, int st, bool useMiai) {
    ...
    for (int r = 0; r < N; r++) {
        ...
        for (int c = 0; c < N - r; c++) {
            ...
            Move mv(st, lcn);
            if ((B.board[lcn] == EMP) &&
                (is_win(B, mv, useMiai, MP, MPsize, false)))
                printf(" *  ");
            else
                printf(" .  ");
        }
...
void getCarrier(struct Board& B, Move mv, int C[], int& csize) {
    ...
    for (int q = 0; q < NumNbrs; q++) {
        ...
        Move mx(mv.s, xLcn);
        if ((B.board[xLcn] == EMP) && (!B.not_in_miai(mx))) C[csize++] = xLcn;
    }
}
...
void Board::release_miai(
    Move mv) {  // WARNING: does not alter miai connectivity
...
void Board::put_stone(Move mv) {
    assert((board[mv.lcn] == EMP) || (board[mv.lcn] == mv.s) ||
           board[mv.lcn] == TMP);
    board[mv.lcn] = mv.s;
}
...
void Board::YborderRealign(Move mv, int& cpt, int c1, int c2, int c3) {
    ...
    //     x             x 3
    assert(near_edge(c1) &&
           near_edge(c2));  // printf("Y border realign %c",emit(s));
    release_miai(Move(mv.s, c1));
    set_miai(mv.s, c1, c3);
    assert(not_in_miai(Move(mv.s, c2)));
...
int Board::moveMiaiPart(Move mv, bool useMiai, int& bdset, int cpt) {
    ...
    if (miReply !=
        lcn) {  // playing into opponent miai... which will be released
        // WARNING: if opp'ts next move is not to the miai response, conn'ty
    ...
    for (int t = 0; t < NumNbrs; t++) {  // look for miai nbrs
        ...
        Move mv1(s, c1);
        Move mv2(s, c2);
        // y border realign: move the miai so stones are connected *and*
        // adjacent to border
        //   * -         becomes  * m
        //  m m *                - m *
        // g g g g              g g g g     <-  border guards
        // rearrange this: 4 cases: both before/after miais possible, only
        // after, only before, none of above
        if (board[nbr] == s && board[c1] == EMP && board[c2] == EMP &&
            ...
            if (!not_in_miai(mv1)) {
                ...
                // int nbr0 = lcn+Bridge_offsets[x-1]; int c0 = lcn +
                // Nbr_offsets[x-1]; if ((board[nbr0]==s) && (board[c0]==EMP) &&
                // (Find(p,nbr0)!=Find(p,cpt))) {
                //// nbr0 also new miai candidate, which is better?
                // int nbrRoot0 = Find(p,nbr0);
                // int b0 = brdr[nbrRoot0] | brdr[cpt];
                // if (numSetBits(b0) > numSetBits(b)) { c2 = c0; nbrRoot =
                // nbrRoot0; }
                //}
                // int nbr2 = lcn+Bridge_offsets[x+1]; int c3 = lcn +
                // Nbr_offsets[x+2]; if ((board[nbr2]==s) && (board[c3]==EMP) &&
                // (Find(p,nbr2)!=Find(p,cpt))) { int nbrRoot2 = Find(p,nbr2);
                // int b2 = brdr[nbrRoot2] | brdr[cpt];
                // int b  = brdr[nbrRoot]  | brdr[cpt];
                // if (numSetBits(b2) > numSetBits(b)) { c1 = c3; nbrRoot =
                // nbrRoot2; }
                nbrRoot = Find(p, nbr);  // int b  = brdr[nbrRoot]  | brdr[cpt];
        ...
        } else if ((board[nbr] == GRD) && (board[c1] == EMP) &&
                   (board[c2] == EMP) && (not_in_miai(mv1)) &&
...
int Board::move(Move mv, bool useMiai, int& bdset) {  // bdset comp. from
                                                      // scratch
    ...
    // WARNING: there are 6 possible miai choices, each consecutive pair
    // conflicts...
    //          right now, selection is made randomly... so might not always
    //          find winning choice .... might be better to pick a maximal set
    //          that maximizes new brdr connectivity
    int nbr, nbrRoot, cpt;
    int lcn = mv.lcn;
    int s = mv.s;
