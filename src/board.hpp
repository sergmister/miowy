...
static inline int nx(int x) {
    return x - 1;
}  // index of x:    black,white are 1,2
...
static inline int fatten(int r, int c) {
    return Np2G * (r + GUARDS) + (c + GUARDS);
}
...
struct Board {                   // for each cell ...
    ...
    // int hist[TotalCells];        //   lcns   of all moves (permutation of
    // lcns) int clr[TotalCells];         //   colors of all moves int numMvs;
    // // number of moves made so far.. with Moves, gives history
    ...
    int moveMiaiPart(Move mv, bool useMiai, int& brdset,
                     int cpt);  // miai part of move
